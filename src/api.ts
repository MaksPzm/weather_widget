export default function API() {
    fetch('https://api.openweathermap.org/data/2.5/weather?lat=55.7505412&lon=37.6174782&appid=7a7246067a4dacd8861ed493fa0284f1&lang=ru&units=metric')
...
function geocodingAPI() {
    fetch('http://api.openweathermap.org/geo/1.0/direct?q=Москва,643&limit=1&appid=7a7246067a4dacd8861ed493fa0284f1')
        .then((response) => response.json())
...
let tempReadings: {temp: number, max: number, min: number, description: string};
...
function result(data: any) {
    ...
    const weatherDescription: string = resultAPI.weather[0].description;
    ...
    localStorage.setItem("description", weatherDescription);
}
